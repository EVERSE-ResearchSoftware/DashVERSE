...
dashboards embedded in a small landing site.

## How the pieces fit

PostgreSQL is the source of truth. JSONB columns hold the rsqa payloads
as-is; views in `database/sql/schema/006_create_views.sql` flatten them
...
by `auth-service` (a small FastAPI app handling registration, login,
...
Engineer, Researcher Who Codes, Trainer). The `landing/` site is a
FastAPI service that embeds those dashboards in iframes for a public
homepage.

OpenTofu (`terraform/`) deploys the whole stack via the Kubernetes and
Helm providers; post-install steps that have to call Superset's REST
API live in Ansible (`ansible/roles/superset_config/`). `just` is the
...
|---|---|
| Change the database schema | `database/sql/schema/00*.sql` |
| Surface a new JSONB field as a column | `database/sql/schema/006_create_views.sql` |
| Validate an assessment field on insert | `validate_assessment_payload()` in `database/sql/schema/004_create_triggers.sql` |
| Add an auth endpoint | `auth-service/app/api/` |
| Add a route to the landing site | `landing/app/api/routes.py` |
| Edit a dashboard or chart | The Superset UI, then `just export-superset-assets`. See [`dashboards.md`](dashboards.md). |
| Add a K8s deployment | New module under `terraform/modules/`, wired in `terraform/main.tf` |
...
- `database/sql/data/004_assessment_examples.sql` is hand-written SQL,
...
- The sync CronJob in `terraform/modules/sync/main.tf` and the Ansible
...
- [`../API_examples.md`](../API_examples.md) -- PostgREST and auth-service curl examples
