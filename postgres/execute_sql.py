...
def truncate_long_text(row, max_length=50):
    return {key: (value[:max_length] + "..." if isinstance(value, str) and len(value) > max_length else value)
...
def execute_sql_script(sql_script, db_config, max_length, save_json, display_table):
    ...
    try:
        ...
        if "select" in sql_script.lower():
            ...
            results_dict = [truncate_long_text(dict(row), max_length=max_length) for row in results]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Execute an SQL script on a PostgreSQL database.")
    parser.add_argument(
        "--sql-file",
        type=str,
    ...
    # Execute the SQL script
    execute_sql_script(sql_script, db_config, args.max_length, args.save_json, args.display_table)
